...
from creature_service import creatures, create_creature, eat_food, get_eyesight, reproduce_if_possible, fight
from food_service import foods, create_food
from grid_service import get_position_as_12_bit_number, get_neighbours, get_foods_in_eyesight, move, move_towards_food
...
def simulate_individual_creature_step(creature):
    ...
    if len(neighbours) != 0:
        target_neighbour = neighbours[randint(0, len(neighbours) - 1)]
    ...
    if get_position_as_12_bit_number(creature) in foods:
        ...
        return
    foods_in_eyesight = get_foods_in_eyesight(get_eyesight(creature))
